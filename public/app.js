...
document.addEventListener('click',e=>{
 ...
 if(a==='openPortfolio')return openPortfolio();
 if(a==='openControl')return openControl();
 if(a==='positionsInfo')return alert('Positions view is connected to the execution/persistence layer in the next transaction lifecycle step.');
 if(a==='backtestInfo')return alert('Backtest is an optional feature mode.');
 if(a==='sort-score')return setSort('score');
 if(a==='sort-risk')return setSort('risk');
 if(a==='filter')return setFilter(el.dataset.filter||'ALL');
 if(a==='analysis')return showAnalysis(el.dataset.mint||'');
