...
export class AwsKmsSigner {
    ...
    keyId;
    publicKey;
    constructor(kms, keyId, publicKey) {
        this.kms = kms;
        this.keyId = keyId;
    ...
    static async fromEnvironment(env = process.env) {
        ...
        if (!keyId)
            throw new Error('AWS_KMS_KEY_ID_REQUIRED');
        if (!region)
            throw new Error('AWS_REGION_REQUIRED');
    ...
    async sign(base64) {
        ...
        if (signerIndex < 0)
            throw new Error('KMS_PUBLIC_KEY_NOT_A_REQUIRED_TRANSACTION_SIGNER');
        ...
        if (!response.Signature || response.Signature.length !== 64)
            throw new Error('KMS_INVALID_ED25519_SIGNATURE');
        tx.signatures[signerIndex] = Uint8Array.from(response.Signature);
        return Buffer.from(tx.serialize()).toString('base64');
    }
}
function ed25519PublicKeyFromDer(der) {
    ...
    if (raw.length !== 32)
        throw new Error('KMS_PUBLIC_KEY_INVALID_LENGTH');
