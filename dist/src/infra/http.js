...
export async function fetchJson(url, init, timeoutMs = 8000, retries = 2) {
    let last;
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        try {
            ...
            if (!r.ok)
                throw new ClassifiedError(`HTTP ${r.status}: ${text.slice(0, 500)}`, classifyHttp(r.status));
            try {
                return JSON.parse(text);
            }
            catch {
                throw new ClassifiedError('Malformed JSON response', 'NON_RETRYABLE');
            }
        }
        catch (e) {
            last = e;
