...
async function prices(c, mints) {
    if (!c.JUPITER_API_KEY)
        throw new Error('JUPITER_API_KEY_REQUIRED_FOR_RISK_PRICING');
...
async function holdings(connection, wallet) {
...
export async function loadLiveRiskState(connection, c, wallet, requestedPositionUsd, outputMint, store) {
    ...
    for (const h of nonStable) {
        ...
        if (price === undefined)
            throw new Error(`RISK_PRICE_UNAVAILABLE:${h.mint}`);
        const usd = Number(h.raw) / 10 ** h.decimals * price;
        if (!Number.isFinite(usd))
            throw new Error(`RISK_EXPOSURE_INVALID:${h.mint}`);
        totalExposureUsd += usd;
        if (h.mint === outputMint)
            tokenExposureUsd += usd;
    }
    const solPrice = priceMap[WSOL];
    if (solPrice === undefined)
        throw new Error('RISK_SOL_PRICE_UNAVAILABLE');
    ...
    for (const trade of confirmed.filter(t => t.tradeSide === 'EXIT').sort((a, b) => a.createdAtMs - b.createdAtMs)) {
        ...
        if (trade.createdAtMs >= todayStart.getTime() && pnl < 0)
