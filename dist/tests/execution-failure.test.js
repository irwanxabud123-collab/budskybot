...
function makeQuote(transaction) {
    return {
        quote: {
...
function makeHarness(opts = {}) {
    ...
    const store = {
        ...
        updateTrade: vi.fn(async (_id, patch) => { updates.push(patch); Object.assign(baseTrade, patch); }),
    };
    const connection = {
    ...
    const signer = { publicKey: baseTrade.walletPublicKey, sign: vi.fn(async () => signed) };
    const config = {
    ...
    const signal = { signalId: 'signal-1', strategyName: 'test', strategyVersion: '1', modelVersion: '1', calibrationVersion: '1', featureVersion: '1', direction: 'BUY', probability: 0.8, rawProbability: 0.8, calibrationStatus: 'CALIBRATED', calibrationSource: 'LIVE_REPLAY', taScore: 80, faScore: null, marketQualityScore: 80, riskScore: 80, expectedValue: 1, entryPrice: 1, stopLoss: 0.9, takeProfit: 1.2, riskReward: 2, sampleSize: 100, dataQuality: 'COMPLETE', regime: 'NORMAL', bullishReasons: [], bearishReasons: [], neutralReasons: [], disclaimer: 'test' };
    const market = { inputMint: 'A'.repeat(32), outputMint: 'B'.repeat(32), inputAmountRaw: 100n, expectedOutputRaw: 90n, timestampMs: Date.now(), source: 'test', freshnessMs: 0, valid: true, priceImpactPct: 0.1, estimatedLiquidityUsd: 100000 };
    const risk = { decision: 'APPROVE', reason: 'test' };
...
describe('UNKNOWN reconciliation worker', () => {
    ...
    it('moves UNKNOWN to CONFIRMED only with confirmed chain evidence', async () => {
        const store = { updateTrade: vi.fn().mockResolvedValue(undefined), append: vi.fn().mockResolvedValue(undefined) };
        const connection = {
    ...
    it('does not manufacture a failure when the signature is still absent from RPC history', async () => {
        const store = { updateTrade: vi.fn(), append: vi.fn() };
