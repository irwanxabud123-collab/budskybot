...
const c = {
    ...
    MIN_SOL_RESERVE_LAMPORTS: 10000000n,
};
const m = { inputMint: 'A'.repeat(32), outputMint: 'B'.repeat(32), inputAmountRaw: 1n, expectedOutputRaw: 1n, timestampMs: Date.now(), source: 'test', freshnessMs: 0, valid: true, priceImpactPct: 0.1, estimatedLiquidityUsd: 5000 };
const s = { disclaimer: 'test', signalId: 's', timestampMs: Date.now(), token: 'B', direction: 'BUY', entryReference: '1', confidence: 1, strategyName: 't', strategyVersion: '1', modelVersion: 't', calibrationVersion: 't', featureVersion: 't', reason: 't', probability: 1, rawProbability: 1, calibrationStatus: 'CALIBRATED', calibrationSource: 'LIVE_REPLAY', taScore: 50, faScore: null, marketQualityScore: 50, riskScore: 50, expectedValue: 1, entryPrice: 1, stopLoss: 0.9, takeProfit: 1.2, riskReward: 2, bullishReasons: [], bearishReasons: [], neutralReasons: [], dataQuality: 'COMPLETE', sampleSize: 100, regime: 'NORMAL' };
const state = (overrides = {}) => ({ totalExposureUsd: 0, tokenExposureUsd: 0, dailyLossUsd: 0, drawdownUsd: 0, failedTrades: 0, consecutiveLosses: 0, lastTradeAtMs: 0, requestedPositionUsd: 10, walletUsdcUsd: 100, walletSolLamports: 1000000000n, riskStateFresh: true, ...overrides });
describe('RiskEngine', () => {
    ...
    it('rejects PAPER_REPLAY in LIVE unless explicitly enabled', () => {
        const paperSignal = { ...s, calibrationSource: 'PAPER_REPLAY' };
