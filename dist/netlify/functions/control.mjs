...
export default async function (req) {
    ...
    if (!c.API_AUTH_TOKEN)
        return Response.json({ error: 'control_plane_requires_api_auth_token' }, { status: 503 });
    try {
        ...
        if (body.action === 'ACTIVATE_LIVE') {
            if (c.MODE !== 'LIVE')
                return Response.json({ error: 'deployment_not_live_capable', detail: 'Deployment base MODE is not LIVE. PAPER remains locked safely.' }, { status: 409 });
            if (!c.LIVE_ENABLED)
                return Response.json({ error: 'deployment_live_gate_locked', detail: 'LIVE_ENABLED is false at deployment level.' }, { status: 409 });
            if (c.EMERGENCY_STOP)
                return Response.json({ error: 'deployment_emergency_stop_active' }, { status: 409 });
        }
        const store = new LiveControlStore(c);
        const state = await store.mutate(body.action, actor);
