...
export default async function (req) {
    ...
    try {
        ...
        const store = new SupabaseEventStore(c), entry = await store.getTradeById(body.positionId);
        if (!entry)
        ...
        if (entry.tradeSide !== 'ENTRY' || !['CONFIRMED', 'RECONCILED'].includes(entry.state))
            return Response.json({ error: 'POSITION_NOT_OPEN', state: entry.state }, { status: 409 });
        ...
        const entryValue = Number(entry.intent.inputAmountRaw) / 1e6;
        const exitValue = Number(quote.quote.outAmount) / 1e6;
        const unrealized = exitValue - entryValue - (entry.feeUsd ?? 0);
        ...
        return Response.json({ error: e instanceof Error ? e.message : 'position_update_failed' }, { status: 500 });
    }
}
