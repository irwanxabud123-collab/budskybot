...
async function solFeeUsd(c, lamports) {
    if (!c.JUPITER_API_KEY || lamports <= 0)
        return lamports === 0 ? 0 : null;
    try {
        const r = await fetch('https://api.jup.ag/price/v3?ids=So11111111111111111111111111111111111111112', { headers: { 'x-api-key': c.JUPITER_API_KEY }, signal: AbortSignal.timeout(5000) });
        if (!r.ok)
            return null;
        const data = await r.json();
        const p = Number(data['So11111111111111111111111111111111111111112']?.usdPrice);
        return Number.isFinite(p) && p > 0 ? (lamports / 1e9) * p : null;
    }
    catch {
        return null;
    }
...
export default async function (req) {
    ...
    try {
        ...
        if (!body.signedTransaction || !body.requestId || !body.wallet)
            return Response.json({ error: 'signedTransaction_requestId_wallet_required' }, { status: 400 });
        ...
        if (!trade)
            return Response.json({ error: 'TRADE_INTENT_NOT_FOUND' }, { status: 404 });
        if (trade.walletPublicKey !== wallet || trade.intent.walletPublicKey !== wallet)
            return Response.json({ error: 'WALLET_INTENT_MISMATCH' }, { status: 422 });
        if (trade.state === 'CONFIRMED' || trade.state === 'RECONCILED')
            return new Response(json({ tradeId: trade.tradeId, state: trade.state, signature: trade.signature }), { status: 200, headers: { 'content-type': 'application/json' } });
        if (trade.state !== 'TRANSACTION_BUILT' && trade.state !== 'SIGNED')
            return Response.json({ error: 'TRADE_STATE_NOT_EXECUTABLE', state: trade.state }, { status: 409 });
        if (trade.requestId !== body.requestId)
            return Response.json({ error: 'REQUEST_ID_MISMATCH' }, { status: 422 });
        ...
        if (walletIndex < 0 || walletIndex >= requiredSigners) {
            await store.releaseLease(lockKey);
            return Response.json({ error: 'WALLET_NOT_REQUIRED_TRANSACTION_SIGNER' }, { status: 422 });
        }
        if (!tx.signatures[walletIndex] || tx.signatures[walletIndex].every(byte => byte === 0)) {
            await store.releaseLease(lockKey);
            return Response.json({ error: 'WALLET_SIGNATURE_MISSING' }, { status: 422 });
        }
        if (!tx.signatures.some(s => s.some(byte => byte !== 0))) {
            await store.releaseLease(lockKey);
            return Response.json({ error: 'TRANSACTION_NOT_SIGNED' }, { status: 422 });
        }
        ...
        for (let i = 0; i < requiredSigners; i++) {
            ...
            if (!signature || !signer || signature.every(byte => byte === 0))
                continue;
        ...
        if (parsed && trade.tradeSide === 'ENTRY') {
            ...
            if (openEvent)
                await new ReplayLogger({ path: c.REPLAY_DATASET_PATH || undefined, sink: store }).record(openEvent);
        }
