...
export default async function () {
    try {
        ...
        if (persistenceConfigured) {
            try {
                const r = await fetch(`${c.SUPABASE_URL}/rest/v1/replay_events?select=event&limit=1`, { headers: { apikey: persistenceKey, Authorization: `Bearer ${persistenceKey}` }, signal: AbortSignal.timeout(5000) });
                persistenceOk = r.ok;
                if (!r.ok)
                    persistenceError = `SUPABASE_HTTP_${r.status}`;
            }
            catch (e) {
                persistenceError = e instanceof Error ? e.message : 'SUPABASE_UNAVAILABLE';
            }
        }
        let control = null;
        ...
        if (persistenceConfigured) {
            try {
                control = await new LiveControlStore(c).get();
            }
            catch (e) {
                controlError = e instanceof Error ? e.message : 'CONTROL_UNAVAILABLE';
            }
        }
        ...
        return Response.json({ ok: false, error: e instanceof Error ? e.message : 'CONFIG_INVALID' }, { status: 503 });
    }
}
