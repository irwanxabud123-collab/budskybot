...
export default async function (req) {
    ...
    try {
        ...
        return Response.json({ error: 'config_invalid' }, { status: 503 });
    }
    ...
    try {
        ...
        if (!authorized(req, c, trade.walletPublicKey))
            return Response.json({ error: 'wallet_auth_required' }, { status: 401 });
        if (!trade.signature)
            return Response.json({ error: 'SIGNATURE_REQUIRED_FOR_RECONCILIATION', state: trade.state }, { status: 409 });
        if (trade.state === 'RECONCILED')
            return new Response(json({ tradeId: trade.tradeId, state: trade.state, signature: trade.signature, reconciled: true }), { status: 200, headers: { 'content-type': 'application/json', 'cache-control': 'no-store' } });
        ...
        if (!chainStatus)
            return new Response(json({ tradeId: trade.tradeId, state: 'UNKNOWN', signature: trade.signature, reconciled: false, reason: 'SIGNATURE_NOT_VISIBLE_YET' }), { status: 202, headers: { 'content-type': 'application/json' } });
        if (chainStatus.err) {
        ...
        if (!parsed?.meta)
            return new Response(json({ tradeId: trade.tradeId, state: 'UNKNOWN', signature: trade.signature, reconciled: false, reason: 'TRANSACTION_METADATA_NOT_VISIBLE_YET' }), { status: 202, headers: { 'content-type': 'application/json' } });
        if (parsed.meta.err) {
