...
export default async function(req:Request){
 ...
 if(!authorized(req,c))return Response.json({error:'unauthorized'},{status:401});
 if(c.mode==='LIVE')return Response.json({error:'wrong_endpoint_for_live'},{status:409});
 if(!c.WALLET_PUBLIC_KEY)return Response.json({error:'WALLET_PUBLIC_KEY_REQUIRED_FOR_REAL_RISK_SNAPSHOT'},{status:422});
 try{
  const wallet=new PublicKey(c.WALLET_PUBLIC_KEY);const conn=new Connection(c.RPC_URL,{commitment:'confirmed'});
  const [input,output]=await Promise.all([validateMint(conn,c.INPUT_MINT),validateMint(conn,c.OUTPUT_MINT)]);
  if(!input.valid||!output.valid)return Response.json({error:'token_validation_failed',input,output},{status:422});
  ...
  const now=Date.now();const decision=await buildLiveDecisionContext(c,conn,quote.quote,c.INPUT_MINT,c.OUTPUT_MINT,c.INPUT_DECIMALS,c.OUTPUT_DECIMALS,now);const market=decision.market;
  ...
  const engine=new ExecutionEngine(c,j,store);const key=req.headers.get('x-idempotency-key')||crypto.randomUUID();const trade=await engine.execute(signal,market,approval,key,quote,riskState);
  return new Response(JSON.stringify({signal,risk:approval,trade},(_,v)=>typeof v==='bigint'?v.toString():v),{status:(trade as any).state==='QUOTE_VALIDATED'?200:202,headers:{'content-type':'application/json'}});
 }catch(e){return Response.json({error:e instanceof Error?e.message:'execution_failed'},{status:500});}
}
