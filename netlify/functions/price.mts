...
async function fetchPrices(ids: string[]) {
  ...
  if (!key) throw new Error('JUPITER_API_KEY is required for live price polling');
  ...
  const raw = (await r.json()) as Record<string, { usdPrice?: number; priceChange24h?: number; blockId?: number }>;
  const data = {
    updatedAtMs: now,
    prices: ids.reduce<Record<string, { usdPrice: number | null; priceChange24h: number | null }>>((acc, id) => {
      const row = raw[id];
      acc[id] = {
