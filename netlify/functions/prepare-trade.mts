...
import type { MarketSnapshot, Signal, TradeRecord } from '../../src/domain/types.js';
...
function parseUsdcAmount(value: unknown): { display: string; raw: bigint } {
...
async function tokenInfo(c: any, mint: string) {
  ...
  if (!r.ok) throw new Error(`TOKEN_INFO_${r.status}`);
  const rows = await r.json();
  const row = Array.isArray(rows) ? rows.find((x: any) => x.id === mint) : null;
...
export default async function (req: Request) {
  ...
  if (c.MODE !== 'LIVE' || !c.LIVE_ENABLED || c.EMERGENCY_STOP || !c.LIVE_BROWSER_EXECUTION_ENABLED) return Response.json({ error: 'live_browser_execution_not_enabled' }, { status: 409 });
  ...
  if (c.WALLET_SIGNING_MODE !== 'BROWSER') return Response.json({ error: 'browser_signing_mode_required' }, { status: 409 });
  ...
  let walletLease: { store: SupabaseEventStore; lockKey: string; tradeId: string } | null = null;
  let keepWalletLease = false;
  try {
    const body = await req.json() as { wallet?: string; outputMint?: string; amountUsdc?: string | number };
    if (!body.wallet || !body.outputMint || body.amountUsdc === undefined) return Response.json({ error: 'wallet_outputMint_amountUsdc_required' }, { status: 400 });
    ...
    if (existing) {
      if (existing.walletPublicKey !== wallet || existing.intent.walletPublicKey !== wallet) return Response.json({ error: 'WALLET_INTENT_MISMATCH' }, { status: 422 });
    ...
    if (!(await store.claimLease(lockKey, lockOwner, 120000))) return Response.json({ error: 'WALLET_EXECUTION_BUSY' }, { status: 409 });
    walletLease = { store, lockKey, tradeId: lockOwner };
    if (outputMint === USDC) return Response.json({ error: 'output_mint_must_differ_from_usdc' }, { status: 400 });

    const amount = parseUsdcAmount(body.amountUsdc);
    const maxPositionRaw = BigInt(Math.floor(c.MAX_POSITION_USD * 1e6));
    if (amount.raw > maxPositionRaw) return Response.json({ error: 'max_position_size_exceeded', maximumUsd: c.MAX_POSITION_USD }, { status: 422 });
    ...
    if (!inputValid.valid || !outputValid.valid) return Response.json({ error: 'token_validation_failed', input: inputValid, output: outputValid }, { status: 422 });
    const tokenPolicy = await evaluateTokenPolicy(conn, outputValid, defaultTokenPolicy(c.BLACKLISTED_MINTS));
    if (!tokenPolicy.allowed) return Response.json({ error: 'token_policy_rejected', policy: tokenPolicy }, { status: 422 });
    if (Number(token.liquidity || 0) < c.MIN_LIQUIDITY_USD) return Response.json({ error: 'liquidity_too_low', liquidityUsd: Number(token.liquidity || 0), minimum: c.MIN_LIQUIDITY_USD }, { status: 422 });
    ...
    if (signal.direction !== 'BUY') return Response.json({ error: 'STRATEGY_REJECTED', signal, calibrationSource: decision.calibrationSource }, { status: 422 });
    ...
    if (approval.decision !== 'APPROVE') return Response.json({ error: 'risk_rejected', approval }, { status: 422 });
    ...
    if (walletLease && !keepWalletLease) {
      try { await walletLease.store.releaseLease(walletLease.lockKey); } catch { /* best-effort cleanup; lease expiry is the fallback */ }
    }
  }
}
