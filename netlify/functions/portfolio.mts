...
export default async (req:Request)=>{
 ...
 if(!walletParam) return new Response(JSON.stringify({error:'WALLET_REQUIRED'}),{status:400,headers:{'content-type':'application/json'}});
 let c; try { c=loadConfig(); } catch(e) { return new Response(JSON.stringify({error:e instanceof Error?e.message:'CONFIG_INVALID'}),{status:503,headers:{'content-type':'application/json'}}); }
 let wallet; try { wallet=new PublicKey(walletParam).toBase58(); } catch { return new Response(JSON.stringify({error:'INVALID_WALLET'}),{status:400,headers:{'content-type':'application/json'}}); }
 if(!walletSessionAuthorized(req,c,wallet)) return new Response(JSON.stringify({error:'WALLET_AUTH_REQUIRED'}),{status:401,headers:{'content-type':'application/json'}});
 try{
