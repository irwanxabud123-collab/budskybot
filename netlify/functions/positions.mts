...
export default async function(req: Request) {
  ...
  if(!walletParam) return Response.json({error:'wallet_required'}, {status:400});
  let wallet; try { wallet=new PublicKey(walletParam).toBase58(); } catch { return Response.json({error:'invalid_wallet'}, {status:400}); }
  ...
  try {
    ...
    for(const t of trades.filter(x=>x.state==='CONFIRMED'||x.state==='RECONCILED')) {
      ...
      if(t.tradeSide==='ENTRY'){ g.entryTradeId=t.tradeId; g.mint=t.intent.outputMint; g.quantityRaw=(t.positionQuantityRaw??t.intent.expectedOutputRaw).toString(); g.notionalUsd=t.notionalUsd??Number(t.intent.inputAmountRaw)/1e6; g.signature=t.signature??g.signature; }
      if(t.tradeSide==='EXIT'){ g.exitTradeId=t.tradeId; g.state='CLOSED'; g.realizedPnlUsd=t.realizedPnlUsd??null; g.signature=t.signature??g.signature; }
      groups.set(id,g);
    }
  ...
  } catch(e) { return Response.json({error:e instanceof Error?e.message:'POSITIONS_FAILED'},{status:503}); }
}
