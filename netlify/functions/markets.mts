...
async function enrichTechnical(opportunities:Opportunity[]):Promise<Opportunity[]>{
  ...
  for(const o of targets){
    ...
    if(hit&&Date.now()-hit.at<TECHNICAL_TTL){enriched.set(o.mint,hit.value);continue;}
    try{
...
async function scan(){
  ...
  const data={updatedAtMs:now,source:'Jupiter Tokens API V2',universeSize:opportunities.length,opportunities,discovery:{trending:Array.isArray(trending)?trending.slice(0,30):[],traded:Array.isArray(traded)?traded.slice(0,30):[],organic:Array.isArray(organic)?organic.slice(0,30):[]}};
