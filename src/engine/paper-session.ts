import type {MarketSnapshot,Signal,QuoteSummary} from '../domain/types.js';
import {PaperBroker,type PaperPosition, type PaperClosedPosition} from './paper.js';
...
export class PaperTradingSession {
 ...
 constructor(private readonly broker:PaperBroker,private readonly logger:ReplayLogger,private readonly wallet:string,private readonly inputDecimals:number,private readonly outputDecimals:number){}
 hasPosition(){return this.position!==null;}
 getPosition(){return this.position;}
 async apply(signal:Signal,market:MarketSnapshot,quote:QuoteSummary):Promise<{action:'OPEN'|'CLOSE'|'HOLD';pnlUsd:number|null;tradeId:string|null}> {
   if(signal.direction==='BUY' && !this.position){
     const tradeId=id('paper'), positionId=id('paper-pos');
     const trade:TradeRecord={walletPublicKey:this.wallet,tradeId,signalId:signal.signalId,executionId:id('paper-exec'),state:'CONFIRMED',createdAtMs:Date.now(),intent:{walletPublicKey:this.wallet,tradeId,signalId:signal.signalId,inputMint:market.inputMint,outputMint:market.outputMint,inputAmountRaw:market.inputAmountRaw,expectedOutputRaw:market.expectedOutputRaw,maxSlippageBps:quote.slippageBps,timestampMs:market.timestampMs,riskApproved:true},quote,positionId,tradeSide:'ENTRY'};
   ...
   if(signal.direction==='SELL' && this.position){
     const pos=this.position; const tradeId=id('paper'), trade:TradeRecord={walletPublicKey:this.wallet,tradeId,signalId:signal.signalId,executionId:id('paper-exec'),state:'CONFIRMED',createdAtMs:Date.now(),intent:{walletPublicKey:this.wallet,tradeId,signalId:signal.signalId,inputMint:market.inputMint,outputMint:market.outputMint,inputAmountRaw:market.inputAmountRaw,expectedOutputRaw:market.expectedOutputRaw,maxSlippageBps:quote.slippageBps,timestampMs:market.timestampMs,riskApproved:true},quote,positionId:pos.positionId,tradeSide:'EXIT'};
     const closed:PaperClosedPosition=this.broker.close(pos,signal,market,this.outputDecimals,this.inputDecimals);
