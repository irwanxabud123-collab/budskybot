...
import {fitIsotonic,calibrationMetrics,serializeCalibrationModel,type SerializedCalibrationModel} from './calibration.js';
import {computeTA,taScore,type Candle,type TAFeatures} from './ta.js';

export {Candle};

export interface BacktestTrade {
 entryTimestampMs:number; exitTimestampMs:number; entryPrice:number; exitPrice:number;
 grossPnl:number; fees:number; slippageCost:number; netPnl:number; returnPct:number;
 signal:'BUY'|'SELL'; exitReason:'STOP_LOSS'|'TAKE_PROFIT'|'END_OF_DATA'; riskBudgetUsd:number; positionNotionalUsd:number;
}
...
export interface BacktestSeedCalibration {
 source:'BACKTEST_SEED';
 strategyVersion:'budsky-v9.0.0';
 featureVersion:'ta-fa-v1.1';
 methodology:'ATR_TRADE_OUTCOME';
 trainedThroughTimestampMs:number;
 trainingSamples:number;
 model:SerializedCalibrationModel;
}

export interface BacktestMetrics {
 methodology:'canonical-v9-strategy'; lookaheadBiasMitigated:boolean;
 periodStartMs:number|null; periodEndMs:number|null; metricPeriod:'PER_TRADE_NOT_ANNUALIZED';
 trades:number; wins:number; losses:number; winRatePct:number; lossRatePct:number;
 averageWin:number; averageLoss:number; riskRewardActual:number|null; grossPnl:number; fees:number; slippageCost:number;
 netPnl:number; totalReturnPct:number; maxDrawdown:number; maxDrawdownPct:number; expectancyPerTrade:number;
 profitFactor:number|null; sharpeRatio:number|null; sortinoRatio:number|null; calmarRatio:number|null;
 sampleSufficient:boolean; brierScore:number|null; ece:number|null;
 calibrationStatus:'CALIBRATED'|'INSUFFICIENT_DATA'; sampleSize:number;
 riskPerTradePct:number; maxPositionUsd:number; maxTotalExposureUsd:number;
 seedCalibration:BacktestSeedCalibration|null; limitations:string[]; tradesDetail:BacktestTrade[];
}

function csvLine(line:string){const out:string[]=[];let cur='',quoted=false;for(const ch of line){if(ch==='"')quoted=!quoted;else if(ch===','&&!quoted){out.push(cur);cur='';}else cur+=ch;}out.push(cur);return out;}
export async function loadCsv(path:string):Promise<Candle[]>{
 const text=await readFile(path,'utf8');const lines=text.trim().split(/\r?\n/);if(lines.length<2)return[];
 const h=csvLine(lines[0]??'').map(x=>x.trim().toLowerCase());const idx=(n:string)=>h.indexOf(n);
 const ti=idx('timestamp'),oi=idx('open'),hi=idx('high'),li=idx('low'),ci=idx('close'),vi=idx('volume');
 if([ti,oi,hi,li,ci].some(x=>x<0))throw new Error('BACKTEST_CSV_REQUIRES_TIMESTAMP_OPEN_HIGH_LOW_CLOSE');
 const rows:Candle[]=[];
 for(const line of lines.slice(1)){
  ...
  if(Object.values(row).every(Number.isFinite)&&row.open>0&&row.high>0&&row.low>0&&row.close>0)rows.push(row);
 }
...
const std=(v:number[])=>{if(v.length<2)return 0;const m=mean(v);return Math.sqrt(v.reduce((s,x)=>s+(x-m)**2,0)/(v.length-1));};

function featureAt(candles:Candle[],i:number):{ta:TAFeatures;raw:number;bullish:boolean}|null{
 const ta=computeTA(candles.slice(0,i),candles[i]?.timestampMs);
 const score=taScore(ta);if(score===null)return null;
...
interface Candidate {signalIndex:number;entryIndex:number;entry:number;stop:number;target:number;raw:number;}

function discoverCandidates(candles:Candle[],atrStopMultiple:number,rewardRisk:number,threshold:number):Candidate[]{
 const out:Candidate[]=[];
 for(let i=220;i<candles.length-1;i++){
  const f=featureAt(candles,i);const next=candles[i+1];if(!f||!next||!f.bullish||f.raw<threshold)continue;
  const atr=f.ta.atr14.value;if(atr===null||!Number.isFinite(atr)||atr<=0)continue;
...
function outcomeForCandidate(candles:Candle[],c:Candidate,maxExitIndex=candles.length-1):{win:boolean;exitIndex:number;exitPrice:number;reason:BacktestTrade['exitReason']} {
 ...
 for(let j=c.entryIndex;j<=end;j++){
  ...
  if(stopHit)return {win:false,exitIndex:j,exitPrice:c.stop,reason:'STOP_LOSS'};
  if(targetHit)return {win:true,exitIndex:j,exitPrice:c.target,reason:'TAKE_PROFIT'};
 }
 const last=candles[end]!;
 return {win:last.close>c.entry,exitIndex:end,exitPrice:last.close,reason:'END_OF_DATA'};
}

export function runSimpleBacktest(candles:Candle[],initialCapital:number,feeBps:number,slippageBps:number,options:BacktestOptions={}):BacktestMetrics {
 ...
 const base={periodStartMs:candles[0]?.timestampMs??null,periodEndMs:candles.at(-1)?.timestampMs??null,metricPeriod:'PER_TRADE_NOT_ANNUALIZED' as const,riskPerTradePct,maxPositionUsd,maxTotalExposureUsd};
 if(candles.length<250)return {...base,methodology:'canonical-v9-strategy' as const,lookaheadBiasMitigated:true,trades:0,wins:0,losses:0,winRatePct:0,lossRatePct:0,averageWin:0,averageLoss:0,riskRewardActual:null,grossPnl:0,fees:0,slippageCost:0,netPnl:0,totalReturnPct:0,maxDrawdown:0,maxDrawdownPct:0,expectancyPerTrade:0,profitFactor:null,sharpeRatio:null,sortinoRatio:null,calmarRatio:null,sampleSufficient:false,brierScore:null,ece:null,calibrationStatus:'INSUFFICIENT_DATA' as const,sampleSize:0,seedCalibration:null,limitations:['At least 250 candles are required for the canonical feature set.','This historical candle backtest cannot provide external FA/news/social or historical Jupiter execution evidence unless the input dataset contains it.'],tradesDetail:[]};
 ...
 for(const c of test){
  if(c.entryIndex<nextAllowedIndex)continue;
  const calibrated=model?.predict(c.raw)??null;
  if(calibrated===null||calibrated<0.55)continue;
  ...
  cash+=net;totalFees+=entryFee+exitFee;totalSlip+=slip;
  ...
  peak=Math.max(peak,cash);maxDd=Math.max(maxDd,peak-cash);
 }
 ...
 const seedCalibration:BacktestSeedCalibration|null=model&&seedSerialized?{source:'BACKTEST_SEED',strategyVersion:'budsky-v9.0.0',featureVersion:'ta-fa-v1.1',methodology:'ATR_TRADE_OUTCOME',trainedThroughTimestampMs:candles[Math.max(0,cutoffIndex-1)]?.timestampMs??candles[Math.max(0,Math.floor(candles.length*0.7))]!.timestampMs,trainingSamples:trainResolved.length,model:seedSerialized}:null;
