...
import type { SupabaseEventStore } from '../persistence/event-store.js';
...
function isTerminal(state: TxState): boolean {
  return new Set<TxState>([
 ...
 */
export async function reconcileUnknownTrade(
  trade: TradeRecord,
...
export async function reconcileUnknownTrades(
  connection: Connection,
  store: SupabaseEventStore,
  ...
  for (const trade of trades) {
    ...
    try {
      ...
      results.push(result);
      if (result.state === 'CONFIRMED') reconciled++;
      else if (result.state === 'CONFIRMATION_FAILED') failed++;
      else stillUnknown++;
    } catch (e) {
      stillUnknown++;
      ...
      try { await store.releaseLease(lockKey); } catch { /* do not mask reconciliation result */ }
    }
  }
