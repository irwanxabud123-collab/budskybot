...
import type {QuoteSummary, TradeRecord, RiskApproval, MarketSnapshot, RiskState} from '../domain/types.js';
import type {ReplayEvent, DataProvenance, ExecutionMode} from './dataset.js';
...
export function realizedOutputSlippageBps(parsed:ParsedTransactionWithMeta|null,wallet:string,outputMint:string,quotedOut:bigint):number|null{
  const actual=tokenDelta(parsed,wallet,outputMint); if(actual===null||quotedOut<=0n||actual<0n)return null;
  return Number((quotedOut-actual)*10000n/quotedOut)/100;
}
export function actualFeeLamports(parsed:ParsedTransactionWithMeta|null):number|null{return parsed?.meta?.fee===undefined?null:parsed.meta.fee;}

export interface ReplayContext { executionMode?:ExecutionMode; market?:MarketSnapshot|null; risk?:RiskApproval|null; riskState?:RiskState|null; token?:unknown; strategy?:{name:string;version:string;modelVersion?:string;calibrationVersion?:string;featureVersion?:string;action:string;signalId?:string;probability?:number|null;rawProbability?:number|null;calibrationStatus?:string;calibrationSource?:string;taScore?:number|null;faScore?:number|null;marketQualityScore?:number|null;riskScore?:number|null;expectedValue?:number|null;entry?:number|null;stopLoss?:number|null;takeProfit?:number|null;riskReward?:number|null;sampleSize?:number|null;dataQuality?:string;regime?:string;bullishReasons?:string[];bearishReasons?:string[];neutralReasons?:string[]}; positionId?:string|null; grossPnlUsd?:number|null; netPnlUsd?:number|null; notionalUsd?:number|null; parsed?:ParsedTransactionWithMeta|null; confirmationStatus?:string|null; bid?:number|null; ask?:number|null; spreadBps?:number|null; volatility1h?:number|null; volatility24h?:number|null; depth1pct?:number|null; poolTVL?:number|null; }

export function makeReplayEvent(args:{trade:TradeRecord;quote:QuoteSummary|null;wallet:string;eventType?:ReplayEvent['event_type'];side?:'BUY'|'SELL'|'HOLD'|null;executionLatencyMs?:number|null;success?:boolean|null;errorCode?:string|null;actualSlippageBps?:number|null;feeUsd?:number|null;feeLamports?:number|null;context?:ReplayContext}):ReplayEvent{
 const {trade,quote}=args; const c=args.context??{}; const observed=(x:unknown):DataProvenance=>x===null||x===undefined?'MISSING':'OBSERVED';
 const bid=c.bid??null, ask=c.ask??null, mid=bid!==null&&ask!==null?(bid+ask)/2:null;
 const ms={bid,ask,mid,spread_bps:c.spreadBps??(bid!==null&&ask!==null?((ask-bid)/((bid+ask)/2))*10000:null),source:c.market?.source??null,provenance:bid!==null&&ask!==null?'OBSERVED' as DataProvenance:'MISSING' as DataProvenance};
...
function extractArray(raw:unknown,key:string):unknown[]|null{if(!raw||typeof raw!=='object')return null;const v=(raw as Record<string,unknown>)[key];return Array.isArray(v)?v:null;}
