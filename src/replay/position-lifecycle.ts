...
import { makeReplayEvent, type ReplayContext } from './runtime-event.js';
...
function tokenDelta(parsed:ParsedTransactionWithMeta|null,wallet:string,mint:string):bigint|null{
  if(!parsed?.meta)return null;
  const pre=(parsed.meta.preTokenBalances??[]).filter(x=>x.owner===wallet&&x.mint===mint).reduce<bigint>((n,x)=>n+BigInt(x.uiTokenAmount.amount),0n);
  const post=(parsed.meta.postTokenBalances??[]).filter(x=>x.owner===wallet&&x.mint===mint).reduce<bigint>((n,x)=>n+BigInt(x.uiTokenAmount.amount),0n);
  return post-pre;
}
function solDelta(parsed:ParsedTransactionWithMeta|null,wallet:string):bigint|null{
  if(!parsed?.meta||!parsed.transaction.message.accountKeys)return null;
...
function usdFromSolLamports(lamports:bigint, quoteOutRaw:bigint, quoteInRaw:bigint, quoteInputMint:string, quoteOutputMint:string):number|null{
  if(quoteInputMint===USDC_MINT&&quoteOutputMint===SOL_MINT&&quoteOutRaw>0n){return Number(lamports)/1e9*(Number(quoteInRaw)/1e6)/(Number(quoteOutRaw)/1e9);}
  if(quoteInputMint===SOL_MINT&&quoteOutputMint===USDC_MINT&&quoteInRaw>0n){return Number(lamports)/1e9*(Number(quoteOutRaw)/1e6)/(Number(quoteInRaw)/1e9);}
  return null;
}
function eventWithPnl(base:ReturnType<typeof makeReplayEvent>,patch:Partial<ReplayEvent>):ReplayEvent{return {...base,...patch,pnl:{...base.pnl,...(patch.pnl??{})}} as ReplayEvent;}

export function positionQuantityFromParsed(parsed:ParsedTransactionWithMeta|null,wallet:string,mint:string):bigint|null{return tokenDelta(parsed,wallet,mint);}

export function buildPositionOpenEvent(trade:TradeRecord,wallet:string,parsed:ParsedTransactionWithMeta,logger:ReplayLogger):ReplayEvent|null{
  const output=tokenDelta(parsed,wallet,trade.intent.outputMint); const input=tokenDelta(parsed,wallet,trade.intent.inputMint); if(output===null||input===null)return null;
  ...
  const ctx:ReplayContext={positionId:trade.positionId??trade.tradeId,parsed,confirmationStatus:'confirmed',notionalUsd:entryValueUsd};
...
export function buildPositionCloseEvent(args:{entry:TradeRecord;exit:TradeRecord;wallet:string;entryParsed:ParsedTransactionWithMeta;exitParsed:ParsedTransactionWithMeta;logger:ReplayLogger;entryFeeUsd?:number|null;exitFeeUsd?:number|null}):ReplayEvent|null{
 const {entry,exit,wallet,entryParsed,exitParsed}=args;
 const entryUsdc=tokenDelta(entryParsed,wallet,USDC_MINT); const exitUsdc=tokenDelta(exitParsed,wallet,USDC_MINT);
 const entryToken=tokenDelta(entryParsed,wallet,entry.intent.outputMint); const exitToken=tokenDelta(exitParsed,wallet,entry.intent.outputMint);
 if(entryUsdc===null||exitUsdc===null||entryToken===null||exitToken===null)return null;
 const entryValueUsd=-Number(entryUsdc)/1_000_000; const exitValueUsd=Number(exitUsdc)/1_000_000;
 const gross=exitValueUsd-entryValueUsd;
 const entryFeeUsd=args.entryFeeUsd??entry.feeUsd??null; const exitFeeUsd=args.exitFeeUsd??exit.feeUsd??null;
 if(entryFeeUsd===null||exitFeeUsd===null)return null;
 ...
 const totalSlippage=entrySlippageUsd+exitSlippageUsd; const totalFee=entryFeeUsd+exitFeeUsd;
 ...
 const ctx:ReplayContext={positionId:entry.positionId??entry.tradeId,parsed:exitParsed,confirmationStatus:'confirmed',notionalUsd:Math.abs(entryValueUsd)};
...
export async function recordPositionUpdate(args:{trade:TradeRecord;wallet:string;quote:TradeRecord['quote'];market?:ReplayContext['market'];logger:ReplayLogger;unrealizedPnlUsd:number|null}):Promise<void>{
