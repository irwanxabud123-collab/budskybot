...
const bool = (fallback: boolean) => z.string().optional().default(String(fallback)).transform(v => v.toLowerCase() === 'true');
...
const schema = z.object({
  ...
  if (c.MAX_DAILY_LOSS_USD > c.MAX_DRAWDOWN_USD) ctx.addIssue({code:'custom',path:['MAX_DAILY_LOSS_USD'],message:'must not exceed MAX_DRAWDOWN_USD'});
  if (c.MAX_TOKEN_EXPOSURE_USD > c.MAX_TOTAL_EXPOSURE_USD) ctx.addIssue({code:'custom',path:['MAX_TOKEN_EXPOSURE_USD'],message:'must not exceed MAX_TOTAL_EXPOSURE_USD'});
  if (c.NETWORK === 'mainnet-beta' && !c.RPC_URL.startsWith('https://')) ctx.addIssue({code:'custom',path:['RPC_URL'],message:'mainnet RPC must use HTTPS'});
  if (c.MODE === 'LIVE' && !c.JUPITER_API_KEY) ctx.addIssue({code:'custom',path:['JUPITER_API_KEY'],message:'LIVE requires JUPITER_API_KEY'});
  if (c.MODE === 'LIVE' && !c.API_AUTH_TOKEN) ctx.addIssue({code:'custom',path:['API_AUTH_TOKEN'],message:'LIVE requires API_AUTH_TOKEN'});
  if (c.PAPER_REPLAY_MAX_POSITION_USD > c.MAX_POSITION_USD) ctx.addIssue({code:'custom',path:['PAPER_REPLAY_MAX_POSITION_USD'],message:'must not exceed MAX_POSITION_USD'});
  if (c.BACKTEST_SEED_MAX_POSITION_USD > c.MAX_POSITION_USD) ctx.addIssue({code:'custom',path:['BACKTEST_SEED_MAX_POSITION_USD'],message:'must not exceed MAX_POSITION_USD'});
  if (c.MODE === 'LIVE' && c.MAX_POSITION_USD > c.MAX_TOTAL_EXPOSURE_USD) ctx.addIssue({code:'custom',path:['MAX_POSITION_USD'],message:'must not exceed MAX_TOTAL_EXPOSURE_USD'});
  if (c.MODE === 'LIVE' && !c.LIVE_ENABLED) ctx.addIssue({code:'custom',path:['LIVE_ENABLED'],message:'LIVE requires LIVE_ENABLED=true'});
  if (c.MODE === 'LIVE' && c.EMERGENCY_STOP) ctx.addIssue({code:'custom',path:['EMERGENCY_STOP'],message:'emergency stop active'});
  if (c.MODE === 'LIVE' && c.WALLET_SIGNING_MODE === 'BROWSER' && !c.LIVE_BROWSER_EXECUTION_ENABLED) ctx.addIssue({code:'custom',path:['LIVE_BROWSER_EXECUTION_ENABLED'],message:'browser live execution requires an explicit enable flag'});
  if (c.MODE === 'LIVE' && c.REQUIRE_TRANSACTION_INTENT_ALLOWLIST && c.ALLOWED_PROGRAM_IDS.length === 0) ctx.addIssue({code:'custom',path:['ALLOWED_PROGRAM_IDS'],message:'live requires an explicit transaction program allowlist'});
  if (c.REQUIRE_SUPABASE && (!c.SUPABASE_URL || !(c.SUPABASE_KEY || c.SUPABASE_SECRET_KEY || c.SUPABASE_SERVICE_ROLE_KEY))) ctx.addIssue({code:'custom',path:['SUPABASE_URL'],message:'Supabase persistence is required'});
});
export type Config = z.infer<typeof schema> & { mode: Mode };
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config { const c = schema.parse(env); return {...c, mode: c.MODE}; }
