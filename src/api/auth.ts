...
function base58Decode(value: string): Buffer {
  ...
  for (const ch of value) {
    ...
    if (carry0 < 0) throw new Error('INVALID_BASE58');
    let carry = carry0;
  ...
  for (let i = 0; i < bytes.length; i++) {
    const v = bytes[i];
    if (v === undefined) throw new Error('BASE58_STATE');
...
function cookies(req: Request): Record<string,string> {
  ...
  for (const part of (req.headers.get('cookie') ?? '').split(';')) { const i = part.indexOf('='); if (i > 0) out[part.slice(0,i).trim()] = decodeURIComponent(part.slice(i+1).trim()); }
  return out;
}
function signSession(secret: string, wallet: string, expires: number): string {
  const payload = `${wallet}.${expires}`;
  const mac = createHmac('sha256', secret).update(payload).digest('base64url');
  return `${b64url(Buffer.from(payload))}.${mac}`;
}
function validSession(req: Request, c: Config, wallet: string): boolean {
  if (!c.API_AUTH_TOKEN) return false;
  const value = cookies(req)[SESSION_COOKIE]; if (!value) return false;
  const [payload64, mac] = value.split('.'); if (!payload64 || !mac) return false;
  let payload: string; try { payload = fromB64url(payload64).toString(); } catch { return false; }
  const parts = payload.split('.');
  const sessionWallet = parts[0];
  const expText = parts[1];
  if (typeof sessionWallet !== 'string' || typeof expText !== 'string') return false;
  const exp = Number(expText);
  if (!Number.isFinite(exp) || !Number.isSafeInteger(exp)) return false;
  if (sessionWallet !== wallet || exp < Date.now()) return false;
...
export function authorized(req: Request, c: Config, wallet?: string): boolean {
  if (c.MODE !== 'LIVE') return !c.API_AUTH_TOKEN || req.headers.get('authorization') === `Bearer ${c.API_AUTH_TOKEN}`;
...
export function walletSessionAuthorized(req: Request, c: Config, wallet: string): boolean { return validSession(req, c, wallet); }

export function challengeResponse(message: string): Response {
...
export function issueSession(req: Request, c: Config, wallet: string, message: string): Response | null {
  if (!c.API_AUTH_TOKEN) return null;
...
export function base58Encode(value: Uint8Array): string {
  ...
  let out = '';
  for (const byte of value) { if (byte !== 0) break; out += '1'; }
